...
plot_samples <- function(dat,
  ...
  if (!blank_plot) {
    g <- g + ggplot2::geom_col(data = dat,
      ...
      ggplot2::ylim(0, max(stacked_data$samples, na.rm = TRUE) * 1.05)
  }
  ...
  g <- g +
    ...
    #ggplot2::scale_y_continuous(position = "right", expand = waiver())
  ...
  labs <- unique(select(samples, area))

  .mult <- 0.935 # vertical area label
